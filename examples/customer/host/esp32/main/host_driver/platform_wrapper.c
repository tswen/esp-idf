...
static SemaphoreHandle_t ReadSemaphore = NULL;
...
int control_path_platform_init(void)
{
	...
	if (ESP_OK != serial_if_g->fops->open(serial_if_g)) {
...
{
	if (ESP_OK != serial_if_g->fops->close(serial_if_g)) {
	...
	int read_len, uint8_t wait, uint32_t* buf_len)
{
    ...
    if(xSemaphoreTake(ReadSemaphore, (TickType_t)(wait * 1000)) != pdTRUE ) {
